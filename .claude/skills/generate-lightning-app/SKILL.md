...
metadata:
  version: "1.0"
  related-skills: generate-custom-object, generate-custom-field, generate-custom-tab, generate-flexipage, generate-custom-application, generate-flow, generate-validation-rule, generate-list-view, generate-permission-set
---
...
- User only needs to create or configure an app container (grouping existing tabs) without other metadata; use `generate-custom-application` instead
...
|---------------|------------|-------------|------------|
| **Custom Object** | `generate-custom-object` | `salesforce-api-context` | MUST load skill AND call API context |
| **Custom Field** | `generate-custom-field` | `salesforce-api-context` | MUST load skill AND call API context |
| **Custom Tab** | `generate-custom-tab` | `salesforce-api-context` | MUST load skill AND call API context |
| **FlexiPage** | `generate-flexipage` | `salesforce-api-context` | MUST load skill AND call API context |
| **Custom Application** | `generate-custom-application` | `salesforce-api-context` | MUST load skill AND call API context |
| **List View** | `generate-list-view` | `salesforce-api-context` | MUST load skill AND call API context (if requested) |
| **Validation Rule** | `generate-validation-rule` | `salesforce-api-context` | MUST load skill AND call API context (if requested) |
| **Flow** | `generate-flow` | `metadata-experts` pipeline | MUST load skill AND run pipeline. **Exempt from `salesforce-api-context`**. |
| **Permission Set** | `generate-permission-set` | `salesforce-api-context` | MUST load skill AND call API context |
...
1. `generate-custom-object` - once, with all objects
2. `generate-custom-field` - once, with all fields (including Master-Detail, Lookup, Roll-up Summary)
...
1. `generate-validation-rule` - once, if validation requirements mentioned
2. `generate-flow` - once, if automation/workflow requirements mentioned
...
1. `generate-list-view` - once, for filtered record views (if requested)
2. `generate-custom-tab` - once, with all object tabs
3. `generate-flexipage` - once, with all record/home/app pages
...
1. `generate-custom-application` - once, to create the Lightning App container
...
1. `generate-permission-set` - once, with all permission sets and access to:
...
- CustomObject: load generate-custom-object + call salesforce-api-context
- CustomField: load generate-custom-field + call salesforce-api-context
- ValidationRule: load generate-validation-rule + call salesforce-api-context (if requested)
- Flow: load generate-flow + run metadata-experts pipeline (if requested)
- ListView: load generate-list-view + call salesforce-api-context (if requested)
- CustomTab: load generate-custom-tab + call salesforce-api-context
- FlexiPage: load generate-flexipage + call salesforce-api-context
- CustomApplication: load generate-custom-application + call salesforce-api-context
- PermissionSet: load generate-permission-set + call salesforce-api-context
...
**1. Custom Objects**
- ① Load skill: Read `generate-custom-object` SKILL.md
...
**2. Custom Fields**
- ① Load skill: Read `generate-custom-field` SKILL.md
...
- ① Load skill: Read `generate-validation-rule` SKILL.md
...
- ① Load skill: Read `generate-flow` SKILL.md
...
- ① Load skill: Read `generate-list-view` SKILL.md
...
**6. Custom Tabs**
- ① Load skill: Read `generate-custom-tab` SKILL.md
...
**7. FlexiPages**
- ① Load skill: Read `generate-flexipage` SKILL.md
...
- ① Load skill: Read `generate-custom-application` SKILL.md
...
**9. Permission Sets**
- ① Load skill: Read `generate-permission-set` SKILL.md
