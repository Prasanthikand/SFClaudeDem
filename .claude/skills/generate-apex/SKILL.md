...
2. Choose the smallest correct pattern
    -   For any test class work, always read and use generate-apex-test skill
...
4.	Generate test classes 
    -   Load the skill generate-apex-test to create {ClassName}Test.cls and {ClassName}Test.cls-meta.xml. 
    -   Apex tests are always required to be generated to deploy. 
    -   No test file creation or edits can occur without loading the generate-apex-test skill to generate tests.
...
|----------------------------------------------|-------------------------------------------------------|
| Place all SOQL outside loops queries         | Avoid query governor limts (100)                      |
|------------------------------------------------------------------------------------------------------|
| Place all DML outside loops statements       | Avoid DML governor limits (150)                       |
|------------------------------------------------------------------------------------------------------|
| Declare a sharing keyword on every class     | Prevent unintended without sharing defaults and data  |
|------------------------------------------------------------------------------------------------------|
| Use Custom Metadata/Labels/describe calls    | Ensure portability across orgs                        |
|------------------------------------------------------------------------------------------------------|
| Always handle exceptions                     | Prevent silent failures                               |
|------------------------------------------------------------------------------------------------------|
| Use bind variables for dynamic SOQL          | Prevent SOQL injection                                |
|------------------------------------------------------------------------------------------------------|
| Use Apex-native collections (List, Map, Set) | Prevent compile errors                                |
|------------------------------------------------------------------------------------------------------|
| Verify methods exist in Apex before use      | Prevent reliance on non-existent APIs                 |
|------------------------------------------------------------------------------------------------------|
| Avoid System.debug() in main code paths      | Debugs consume CPU; use logging framework if required |  
|------------------------------------------------------------------------------------------------------|
| Never use @future methods                    | Use Queueable with System.Finalizer; @future has      |
|                                              |  chaining and type limits. @future cannot chain,      |
|                                              |  cannot be called from Batch, and cannot accept       |
|                                              | non-primitive types                                   |
|------------------------------------------------------------------------------------------------------|
...
⚠️ **MANDATORY DISALLOWED**: methods mixing orchestration with inline SOQL/DML/HTTP; business rules mixed with parsing internals; validation + persistence + cross-system plumbing in one method
...
•	{ClassName}Test.cls (generated via generate-apex-test skill)
•	{ClassName}Test.cls-meta.xml (generated via generate-apex-test skill)
...
| Need                        | Delegate to               |
+-----------------------------+---------------------------+
| Apex tests / fix failures   | generate-apex-test skill  |
| Describe objects/fields     | salesforce-admin agent    |
| Deploy to org               | salesforce-devops agent   |
| Flow calling Apex           | salesforce-admin agent    |
| LWC calling Apex            | salesforce-developer agent|
+-----------------------------+---------------------------+

________________________________________
### Troubleshooting Boundary
This skill handles production .cls/.trigger/.apex issues only: compile/parse failures, deployment dependency errors, runtime governor-limit failures. For test execution, assertions, coverage, or sf apex run test failures, delegate to generate-apex-test.